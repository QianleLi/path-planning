...
// Integral parameter
double _max_input_acc     = 1.0;   //最大输入
int    _discretize_step   = 2;      //只离散成两步
...
void trajectoryLibrary(const Vector3d start_pt, const Vector3d start_velocity, const Vector3d target_pt)
{
    ...
    //TrajectoryStatePtr是三维指针数组，TrajectoryState记录了当前这个位置的p和V
    //意在把加速度离散化
    //这里的输入u就是加速度a，要优化的也是加速度，而不是PPT中的jerk
        ...
        for(int j=0;j <= _discretize_step; j++){        //acc_input_ay
            ...
            for(int k=0; k <= _discretize_step; k++){   //acc_input_az
                ...
                for(int step=0 ; step<=_time_step ; step ++){
                    ...
                    use the pos and vel to record the steps in the trakectory
                    */
               ...
               //pos和vel是轨迹末端的位置和速度，那么轨迹开始的位置就是target_pt，而速度就是(0,0,0)
